...
addHook("TouchSpecial", function(special, toucher)
	...
	if toucher and toucher.valid and tplayer and tplayer.valid then
		local lastlap_perplayer = ((CV_PTSR.lappingtype.value == 2) and (tplayer.lapsdid >= CV_PTSR.maxlaps.value))
...
addHook("MobjThinker", function(mobj)
	...
	if displayplayer and displayplayer.valid then
		if ((CV_PTSR.lappingtype.value == 2) and (displayplayer.lapsdid >= CV_PTSR.maxlaps.value)) or not PTSR.pizzatime then
